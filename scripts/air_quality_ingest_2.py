...
Protocol: HJ212 (TCP) & Modbus RTU-over-TCP (lead sensor, dial-in gateway)
...
# ---- Lead sensor gateway (dial-in, e.g. DR314-TCPServer.py's device) ----
# The lead transmitter's RS485-to-Ethernet gateway only runs in TCP CLIENT
# mode: the device dials OUT to us, rather than us connecting to it. So
# lead_ip in the `stations` table is now the IP the device dials in FROM
# (used to match its connection to a station), and lead_port is no longer
# used for connecting out — the device instead dials AQ_LEAD_SERVER_PORT.
# lead_slave is still per-station as before (e.g. 1 for the original lead
# unit, 2 for a reprogrammed DR314 unit).
AQ_LEAD_SERVER_HOST = os.getenv("AQ_LEAD_SERVER_HOST", "0.0.0.0")
# Defaults to 1935 to match DR314-TCPServer.py's own default — but that's
# the SAME port SERVER_PORT uses for HJ212 telemetry above, and two sockets
# can't bind the same host:port. If both run on this host, set this to a
# free port (e.g. 1936) via .env AND update the gateway's own "Remote Port
# Number" on its web page to match.
AQ_LEAD_SERVER_PORT = int(os.getenv("AQ_LEAD_SERVER_PORT", 1935))
...
# ==========================================================
# MODBUS LEAD SENSOR SERVICE — dial-in gateway (DR314-TCPServer.py)
# ==========================================================
# These gateways speak plain Modbus RTU (with CRC) over the raw TCP stream,
# not Modbus TCP framing, so pymodbus's client can't be used here — frames
# are built/parsed by hand, ported straight from DR314-TCPServer.py.
...
def find_dial_in_station(ip: str):
    """Matches an inbound Modbus connection to a station by lead_ip (the IP
    the gateway dials in from). Returns (mn, station) or (None, None)."""
    for mn, station in get_stations().items():
        if station.get("lead_ip") == ip:
            return mn, station
    return None, None


def handle_lead_client(conn, addr):
    ip = addr[0]
    mn, station = find_dial_in_station(ip)
    if mn is None:
        logger.warning(f"[MODBUS] Dial-in connection from {ip} doesn't match any station's lead_ip — closing.")
        conn.close()
        return
    if not station.get("enabled", True):
        logger.warning(f"[MODBUS] Station {mn} (lead IP {ip}) dialed in but is disabled — closing.")
        conn.close()
        return

    slave = station["lead_slave"]
    logger.info(f"[MODBUS] Station {mn} (lead IP {ip}, slave {slave}) connected.")
    try:
        while True:
            query = build_query(slave, POLL_START, POLL_COUNT)
            conn.sendall(query)
            regs = read_and_parse(conn, slave)
            if len(regs) >= 2:
                temperature, lead = regs[0] / 10.0, regs[1] / 10.0
                update_lead_value(mn, ip, lead, temperature)
            time.sleep(LEAD_POLL_INTERVAL)
    except (ConnectionError, socket.timeout) as e:
        logger.info(f"[MODBUS] Station {mn} (lead IP {ip}) disconnected: {e}")
    except ValueError as e:
        logger.error(f"[MODBUS] Station {mn} (lead IP {ip}): {e}")
    finally:
        conn.close()


def start_lead_tcp_server():
    server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    server.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    server.bind((AQ_LEAD_SERVER_HOST, AQ_LEAD_SERVER_PORT))
    server.listen(MAX_CONNECTIONS)
    logger.info(f"Modbus lead dial-in server running on {AQ_LEAD_SERVER_HOST}:{AQ_LEAD_SERVER_PORT}")
    while True:
        conn, addr = server.accept()
        threading.Thread(target=handle_lead_client, args=(conn, addr), daemon=True,
                          name=f"LeadDialIn-{addr[0]}").start()


def start_lead_dial_in_service():
    threading.Thread(target=start_lead_tcp_server, daemon=True, name="LeadDialInServer").start()
