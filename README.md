...
In addition to receiving seismic telemetry via SMS, the SIM800L driver supports
sending SMS messages for acknowledgements, diagnostics, or remote command
responses.

### Implementation

The SIM800L driver (`sim800l.py`) includes a `send_sms()` method that follows
the standard SIMCom AT command sequence for SMS transmission.
...
Unlike most AT commands, `AT+CMGS` returns a standalone `>` prompt rather than
a CR/LF-terminated response. Because of this, the driver implements a dedicated
`_wait_for_prompt()` helper instead of using the normal `_read_until()` parser.

### Added methods

#### `send_sms(number, text, timeout=None)`

Sends an SMS using the SIM800L modem.

The method:

1. Sends `AT+CMGS="<number>"`.
2. Waits for the modem's `>` prompt.
3. Writes the SMS body.
4. Sends **Ctrl+Z (0x1A)** to transmit the message.
5. Waits for the modem to return:
...
A longer timeout is used (`max(self.timeout * 3, 15)`) because GSM message
delivery typically takes longer than ordinary AT commands.

#### `_wait_for_prompt(timeout)`

A dedicated helper that waits for the `>` prompt generated by `AT+CMGS`. This
is necessary because the prompt is not terminated by carriage return or newline
characters and therefore cannot be handled by the standard line-based parser.
...
This enables the gateway to send SMS acknowledgements or responses directly
through the SIM800L modem.

### Validation

The SMS transmission sequence has been verified using a mocked serial interface
to validate the complete AT command flow without requiring physical hardware.
...
This confirms the driver's state machine correctly handles the SIM800L SMS
transmission workflow.

### Hardware verification

Although the transmission logic has been validated with a mocked serial port,
final verification should be performed on a physical SIM800L module.
...
This verifies the complete GSM transmission path, including UART communication,
modem operation, and carrier network delivery.
