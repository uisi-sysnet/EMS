...
class DashboardController extends Controller
{
    ...
    private function drawImgHeader($image, $generatedAt, string $generatedBy, int $x, int $width, int $y): int
    {
        ...
        // Base width used as reference (match your original IMAGE_WIDTH)
        $baseWidth = 1240;
        $scale = $imgWidth / $baseWidth;

        // Scale font sizes and vertical offsets
        $titleSize       = 24 * $scale;
        $subtitleSize    = 13 * $scale;
        $metaSize        = 14 * $scale;
        $titleOffset     = 28 * $scale;      // baseline from top
        $subtitleOffset  = 22 * $scale;      // below title baseline
        $metaTopOffset   = 4 * $scale;       // above title baseline
        $metaBottomOffset= 20 * $scale;      // below title baseline
        $dividerHeight   = 3 * $scale;       // thickness of the line

        // Ensure minimum sizes for readability (optional)
        $minFont = 10;
        $titleSize   = max($titleSize, $minFont * 2);
        $subtitleSize= max($subtitleSize, $minFont);
        $metaSize    = max($metaSize, $minFont);

        // Calculate positions
        $titleBaseline    = $y + $titleOffset;
        $subtitleBaseline = $titleBaseline + $subtitleOffset;

        // Draw title and subtitle (left‑aligned)
        $this->imgText($image, 'Environment Monitoring System Status Report', $x, $titleBaseline, $titleSize, $teal, true);
        $this->imgText($image, 'Developed by Uplink Integrated Solutions Inc.', $x, $subtitleBaseline, $subtitleSize, $gray);

        // Draw metadata right‑aligned
        $this->imgText($image, 'Report Date/Time: ' . $generatedAt->format('F d, Y  h:i A'), $x + $width, $titleBaseline - $metaTopOffset, $metaSize, $dark, false, 'right');
        $this->imgText($image, 'Generated By: ' . $generatedBy, $x + $width, $titleBaseline + $metaBottomOffset, $metaSize, $dark, false, 'right');

        // Divider line
        $dividerY  = $subtitleBaseline + (14 * $scale);
        $tealColor = imagecolorallocate($image, $teal[0], $teal[1], $teal[2]);
        imagefilledrectangle($image, $x, $dividerY, $x + $width, $dividerY + $dividerHeight, $tealColor);

        return $dividerY + $dividerHeight;
    }
