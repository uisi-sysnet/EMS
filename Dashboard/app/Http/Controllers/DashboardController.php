...
class DashboardController extends Controller
{
     ...
     */
    private function ensureSpace(array &$pages, &$page, int $y, int $needed, $generatedAt, int $pageNum): int
    {
        ...
        $this->drawImgFooter(
            ...
            $generatedAt,
            $pageNum,
            0   // totalPages will be filled later (or leave as 0 for now)
        );
        $pages[] = $page;
    ...
    private function drawImgHeader($image, $generatedAt, string $generatedBy, int $x, int $width, int $y): int
    {
        ...
        // Left-aligned report meta
        $metaBaseline1 = $subtitleBaseline + 28;
        $this->imgText($image, 'Report Date/Time:', $x, $metaBaseline1, 14, $gray);
        $this->imgText($image, $generatedAt->format('F d, Y  h:i A'), $x + 175, $metaBaseline1, 14, $gray); // increased offset

        $metaBaseline2 = $metaBaseline1 + 22;
        $this->imgText($image, 'Generated By:', $x, $metaBaseline2, 14, $gray);
        $this->imgText($image, $generatedBy, $x + 175, $metaBaseline2, 14, $gray); // same offset

        // Teal divider
        $dividerY  = $metaBaseline2 + 18;
