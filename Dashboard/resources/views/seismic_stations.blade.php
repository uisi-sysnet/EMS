    ...
    <div class="bg-surface-900 rounded-2xl shadow-xl border border-border-800 overflow-hidden flex-1 flex flex-col min-h-0">
        ...
        <div class="px-4 sm:px-6 py-3.5 sm:py-4 border-b border-border-800 bg-surface-800 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
            <h2 class="text-lg sm:text-xl font-semibold text-text-100 flex items-center gap-2.5">
                <span class="leading-tight uppercase tracking-wide">Manage Seismic Stations</span>
            </h2>
            <span class="text-xs sm:text-sm text-text-400">Create and manage your seismic monitoring stations</span>
        </div>
            ...
            <div class="bg-surface-800 rounded-xl border border-border-700 overflow-hidden flex flex-col shadow-sm">
                ...
                <div class="p-5 border-b border-border-700">
                    <h3 class="text-sm font-bold text-text-100 uppercase tracking-wider mb-4 flex items-center gap-2">
                        ...
                        Add New Seismic Station
                    </h3>

                    <form action="{{ route('seismic-stations.store') }}" method="POST">
                        @csrf

                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            <!-- Station ID -->
                            <div class="flex flex-col">
                                <label for="station_id" class="block text-xs font-medium text-text-400 mb-1.5 uppercase tracking-wide">
                                    Station ID <span class="text-munti-red-400">*</span>
                                </label>
                                <input type="text" id="station_id" name="station_id" value="{{ old('station_id') }}" required
                                    class="w-full px-3.5 py-2.5 border border-border-600 rounded-lg bg-surface-900 text-text-100 placeholder-text-500 focus:ring-2 focus:ring-radar-500/40 focus:border-radar-500 text-sm transition @error('station_id') border-munti-red-500 @enderror">
                                @error('station_id')
                            ...
                            <!-- Elevation -->
                            <div class="flex flex-col">
                                <label for="elevation_m" class="block text-xs font-medium text-text-400 mb-1.5 uppercase tracking-wide">
                                    Elevation (m)
                                </label>
                                <input type="number" step="any" id="elevation_m" name="elevation_m" value="{{ old('elevation_m') }}"
                                    class="w-full px-3.5 py-2.5 border border-border-600 rounded-lg bg-surface-900 text-text-100 placeholder-text-500 focus:ring-2 focus:ring-radar-500/40 focus:border-radar-500 text-sm transition @error('elevation_m') border-munti-red-500 @enderror">
                                @error('elevation_m')
                                    <p class="mt-1 text-xs text-munti-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                    ...
                    <div class="bg-surface-800 rounded-2xl border border-border-700 shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto thin-scrollbar">
                        ...
                        <form id="editForm" method="POST" class="p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                <!-- Station ID -->
                                <div class="flex flex-col">
                                    <label for="edit_station_id" class="block text-xs font-medium text-text-400 mb-1.5 uppercase tracking-wide">
                                        Station ID <span class="text-munti-red-400">*</span>
                                    </label>
                                    <input type="text" id="edit_station_id" name="station_id" required
                                ...
                                <!-- Elevation -->
                                <div class="flex flex-col">
                                    <label for="edit_elevation_m" class="block text-xs font-medium text-text-400 mb-1.5 uppercase tracking-wide">
                                        Elevation (m)
                                    </label>
                                    <input type="number" step="any" id="edit_elevation_m" name="elevation_m"
                                        class="w-full px-3.5 py-2.5 border border-border-600 rounded-lg bg-surface-900 text-text-100 placeholder-text-500 focus:ring-2 focus:ring-radar-500/40 focus:border-radar-500 text-sm transition">
                                </div>
                            </div>
                        ...
                        @if($stations->count())
                            <table class="min-w-full divide-y divide-border-700">
                                <thead class="bg-surface-900/60 text-[11px] uppercase tracking-wider text-text-500 sticky top-0 z-10">
                                    <tr>
                                        <th scope="col" class="px-4 py-3 text-left font-medium">Station ID</th>
                                        ...
                                        <th scope="col" class="px-4 py-3 text-left font-medium">Elevation (m)</th>
                                ...
                                <tbody class="divide-y divide-border-800">
                                    @foreach($stations as $station)
                                        <tr class="hover:bg-surface-700/50 transition" data-station-id="{{ $station->station_id }}">
                                            ...
                                            <td class="px-4 py-2.5 whitespace-nowrap text-center">
                                                <div class="flex items-center justify-center gap-1.5">
                                                    <!-- Edit Button -->
                                                    <button type="button"
                                                            onclick="editStation('{{ $station->station_id }}')"
                                                    ...
                                                    <!-- Delete Button -->
                                                    <button type="button"
                                                            onclick="deleteStation('{{ $station->station_id }}', '{{ $station->station_id }}')"
...
function editStation(stationId) {
    ...
    // Fetch station data
    fetch(`/seismic-stations/${stationId}/edit`)
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            document.getElementById('edit_station_id').value = data.station_id || '';
            document.getElementById('edit_station_name').value = data.station_name || '';
            document.getElementById('edit_latitude').value = data.latitude || '';
            document.getElementById('edit_longitude').value = data.longitude || '';
            document.getElementById('edit_elevation_m').value = data.elevation_m || '';
            document.getElementById('edit_enabled').checked = data.enabled === true;

            // Set form action
            document.getElementById('editForm').action = `/seismic-stations/${stationId}`;
        })
        .catch(error => {
...
function deleteStation(stationId, stationName) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            form.method = 'POST';
            form.action = `/seismic-stations/${stationId}`;
